...
async def compare_access_speed(client: OpenFgaClient, user: str):
    ...
    all_document_ids = [
        ...
        "5",
        "6",
    ]  # For this example, we know we have 6 documents
...
async def test_list_documents():
    ...
    if not store_id:
        ...
        print(
            "Make sure to run fga-setup first and add the store ID to your .env file."
        )
        return
    ...
    try:
        ...
        for user in users:
            ...
            print(
                f"{user} can read documents: {', '.join(readable_docs) if readable_docs else 'None'}"
            )
        ...
        for user in users:
            ...
            print(
                f"{user} can write to documents: {', '.join(writable_docs) if writable_docs else 'None'}"
            )

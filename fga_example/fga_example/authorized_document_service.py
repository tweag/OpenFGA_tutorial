...
async def get_document_by_id(user_id: str, document_id: int) -> Optional[Dict[str, Any]]:
    """
    Get a document by its ID with authorization check.
    ...
    Returns:
        The document as a dictionary or None if not found/authorized
        
    Raises:
        AuthorizationError: If the user does not have permission to read the document
    """
    service = AuthorizedDocumentService(user_id)
    try:
        return await service.get_document_by_id(document_id)
    finally:
        service.close()

async def search_documents(user_id: str, search_term: str) -> List[Dict[str, Any]]:
    """
    Search for documents with authorization filtering.
    ...
    service = AuthorizedDocumentService(user_id)
    try:
        return await service.search_documents(search_term)
    finally:
        service.close()
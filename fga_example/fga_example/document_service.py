...
def require_authorization(relation: str, object_type: str):
    """
    Decorator to check authorization before executing a method.

    Args:
        relation: The relation to check (e.g., 'read', 'write')
        object_type: The type of object (e.g., 'document')
    """
        ...
        async def wrapper(self, object_id: int, *args, **kwargs):
            ...
            if has_access:
                ...
                raise AuthorizationError(
                    f"User {self.user_id} does not have {relation} access to {object_type}:{object_id}"
                )

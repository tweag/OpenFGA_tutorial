...
from openfga_sdk import OpenFgaClient, ClientConfiguration, CreateStoreRequest
...
async def initialize_authorization_model(model_path=None, store_id=None, api_url=None):
    ...
    if store_id is None:
        raise ValueError(
            "store_id must be provided when calling initialize_authorization_model"
        )
...
async def check_access(
    client: OpenFgaClient, user: str, relation: str, object: str
) -> bool:
    """
...
async def list_documents_for_user(
    client: OpenFgaClient, user: str, relation: str = "reader"
) -> List[str]:
    """
...
async def write_tuples(client: OpenFgaClient, to_write: List[dict]):
    ...
    write_response = await client.write(ClientWriteRequest(writes=_tuples), options)
